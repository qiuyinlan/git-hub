...
读也是先说数组类型,再说数组名字
 为什么 argv[i] 是指针？
因为 argv 的每个参数是字符串，字符串本质是一个字符数组，而字符数组的首地址可以用指针表示。
所以，argv[i] 是一个指针，指向该参数字符串的首字符。
(2) argv 是什么？
argv 是一个数组，数组中的每个元素是一个指针。
参数字符串是如何存储的？
编译器和操作系统会在内存中为命令行参数分配空间，存储这些参数的字符串。
argv 中的指针指向这些字符串的起始地址。
>
> ==数组指针==
int (*ptr)[5];  
...
```
dest：目标地址，即数据将被移动到的起始位置。
src：源地址，即数据将被读取的起始位置。
n：要移动的字节数
- 都是前目标,后源,目标是主要关注的
## 注意插入实现逻辑
插入此时是c语言的逻辑,你还要保证原本在那个位置的东西我往后跑
所以是,先把被覆盖掉的移走,再插入,先移动再插
## 变量名唯一性
在同一个作用域内,不能有同名变量
...
## 条件
i=sb->len  这是错的,还要减去1
通常情况下，len 是字符串或缓冲区的有效长度，指针 buf 指向缓冲区的起始位置 buf[0] 到 buf[==len-1==]
这个刚好len个!!

## 逻辑 
如果读到结尾处时空格或者制表符，则填充字符串结束标志'\0'，并减少长度，直到结尾处不是空格或者制表符
## 为什么要减小长度len
因为len代表的是==有效==字符串长度,你填充了\0,就无效了
